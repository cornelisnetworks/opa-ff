...
Release: 1%{?dist}
Summary: Cornelis Omni-Path basic tools and libraries for fabric management.

Group: System Environment/Libraries
License: (GPL-2.0-only AND BSD-3-Clause) OR (GPL-2.0-only AND BSD-2-Clause)
Url: https://github.com/cornelisnetworks/opa-ff
Source0: https://github.com/cornelisnetworks/opa-ff/archive/refs/tags/v%{version}.tar.gz
BuildRequires: gcc
BuildRequires: gcc-c++
...
IFSComponent: Tools_FF %{version}-%{release}
...
Group: System Environment/Libraries
Requires: libibumad%{?_isa}, libibverbs%{?_isa}, openssl%{?_isa}
...
Group: System Environment/Libraries
Requires: rdma-core-devel, openssl-devel, opa-libopamgt
...
%changelog
* Mon Jul 27 2026 Ishan Patel <ipatel@cornelisnetworks.com> - 1:10.14.4.0.20-1
- Initial Fedora packaging submission (STL-69721)
- Fix SPDX license tags to (GPL-2.0-only AND BSD-3-Clause) OR (GPL-2.0-only AND BSD-2-Clause)
- Set unconditional Epoch: 1 to match opa-fm precedent (Fedora rhel macro is unset, so the
  original per-subpackage conditional Epoch blocks never applied on Fedora)
- Switch Source to a Source0: URL pointing at the GitHub release tarball
- Rename package summary/description text; run update_opa_spec.sh's fedora branch to expand
  basic-tools/fastfabric file lists
- hfi1stats is intentionally NOT included in this initial submission (see STL-69721 comments);
  it lives in a separate private repo and will be packaged as a fast-follow
